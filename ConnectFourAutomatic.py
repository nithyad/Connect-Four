...
class Player:
    ...
    def tiebreakMove(self, scores):
        """decides the tie breaking scores"""
        new_list = []
    ...
    def scoresFor(self, b):
        """returns the scores for the moves"""
        scores = [50.0]*b.width
        for col in range(b.width):
            ...
            if self.ply == 0 and col == b.width:
                return scores
            elif self.ply > 0 and b.allowsMove(col):
                b.addMove(col, self.ox)
                opp  = Player(self.oppCh(), self.tiebreakMove, self.ply-1)
                oppScores = opp.scoresFor(b)
                if 100.0 in oppScores:
                    scores[col] = 0.0
                elif 0.0 in oppScores:
                    scores[col] = 100.0
                else:
                    scores[col] = 50.0
                b.delMove(col)
    ...
    def nextMove(self,b):
        """This function returns the column that the player will move to"""
        if( isinstance(self.ply, int)):
            values = self.scoresFor(b)
            move = self.tiebreakMove(values)
            return move
        return input("Where would you like to place a checker? ")